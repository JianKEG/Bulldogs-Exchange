<?php
    ...
    if ($size === '' || $size === 'all') {
        $display = "SELECT 
                    ...
                    product.category, 
                    product.price,
                    GROUP_CONCAT(Product_SizeStock.size SEPARATOR '|') as sizes,
                    GROUP_CONCAT(Product_SizeStock.stock_quantity SEPARATOR '|') as stocks
                    FROM product
        ...
        <div class="flex">
            ...
            <main id="main-content" class="flex-1 p-8 bg-gray-50 text-center">
                ...
                <div class="space-y-4 inline-block bg-white rounded-lg shadow-sm border border-gray-200 overflow-x-auto px-5 pt-5">
                    <form action="../../../actions/admin/product/editProduct.php?id=<?php echo $_GET['id']; ?><?php echo ($size === '' || $size === 'all') ? '' : '&size=' . urlencode($size); ?>" method="POST" class="space-y-4">
                        <?php 
                        while($row = mysqli_fetch_array($sql)){ 
                            $sizes = isset($row['sizes']) && $row['sizes'] ? explode('|', $row['sizes']) : [];
                            $stocks = isset($row['stocks']) && $row['stocks'] ? explode('|', $row['stocks']) : [];
                            $hasNoSize = false;
                            $noSizeStock = 0;
                        ...
                        <input type="text" value="<?php echo $row['category']; ?>" placeholder="Category" class="w-full rounded-lg border-gray-300 border px-3 py-2 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200" disabled>
                        ...
                        <?php } else if ($hasNoSize) { ?>
                        
                        <label class="block text-left text-sm font-medium text-gray-700">Total Stock Quantity</label>
                        <input type="number" name="total_stock" value="<?php echo $noSizeStock; ?>" placeholder="Total Stock" class="w-full rounded-lg border-gray-300 border px-3 py-2 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200">
                        
                        <?php } else { ?>
                        
                        <div class="mb-6">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Sizes and Stock Quantities</label>
                            <div id="size-container" class="space-y-2">
                                <?php 
                                foreach ($sizes as $index => $existingSize) {
                                    if ($existingSize !== 'No Size' && isset($stocks[$index])) {
                                ?>
                                <div class="flex gap-2 mb-2 size-row" data-product-id="<?php echo $row['product_id']; ?>" data-size="<?php echo $existingSize; ?>" data-existing="true">
                                    <select name="sizes[]" class="w-1/2 rounded-lg border-gray-300 border px-3 py-2 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200" onchange="updateSizeOptions()">
                                        <option value="">-- Select Size --</option>
                                        <option value="XS" <?php echo $existingSize === 'XS' ? 'selected' : ''; ?>>XS</option>
                                        <option value="S" <?php echo $existingSize === 'S' ? 'selected' : ''; ?>>S</option>
                                        <option value="M" <?php echo $existingSize === 'M' ? 'selected' : ''; ?>>M</option>
                                        <option value="L" <?php echo $existingSize === 'L' ? 'selected' : ''; ?>>L</option>
                                        <option value="XL" <?php echo $existingSize === 'XL' ? 'selected' : ''; ?>>XL</option>
                                    </select>
                                    <input type="number" name="stocks[]" value="<?php echo $stocks[$index]; ?>" placeholder="Stock Quantity" class="w-1/2 rounded-lg border-gray-300 border px-3 py-2 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200">
                            ...
                            <button type="button" onclick="addSizeRow()" class="mt-2 mb-4 text-sm text-blue-600 hover:underline font-medium cursor-pointer">
                                + Add Size
                            </button>
                        </div>
